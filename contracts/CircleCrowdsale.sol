...
contract CircleCrowdsale is ERC223ReceivingContract {
    ...
    function CircleCrowdsale(address token, uint start, uint end, uint price, uint limit) public {
        _token = Token(token);
        _start = start;
        _end = end;
        _price = price;
    ...
    function buyFor(address beneficiary) public available
