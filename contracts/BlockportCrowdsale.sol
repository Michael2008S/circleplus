...
//
//
contract BlockportCrowdsale is CappedCrowdsale, TimedCrowdsale, FinalizableCrowdsale, CrowdsaleWhitelist, Pausable {
    ...
    function BlockportCrowdsale(uint256 _cap, uint256 _startTime, uint256 _endTime, uint256 _rate, address _wallet, ERC20 _tokenAddress, address _teamVault, address _companyVault)
    CappedCrowdsale(_cap)
    TimedCrowdsale(_startTime, _endTime)
    Crowdsale(_rate, _wallet, _tokenAddress) public {
...
// TODO    // overriding Crowdsale#validPurchase to add extra cap logic
...
//    }
    ...
    function finalization() internal {
        ...
        // token.finishMinting();
        // No more tokens can be added from now
        bpToken.unpause();
        // ERC20 transfer functions will work after this so trading can start.
        super.finalization();
        // finalise up in the tree
    ...
    function setCrowdsaleDates(uint256 _startTime, uint256 _endTime) public onlyOwner returns (bool) {
        require(openingTime > block.timestamp);
        // current startTime in the future
        ...
        InitialDateChange(openingTime, closingTime);
        return true;
    }
